...
class _OrderAnalyticsScreenState extends State<OrderAnalyticsScreen> {
    ...
    Widget build(BuildContext context) {
      ...
      final completed = orders.where((order) => order.status == OrderStatus.pickedUp).length;
      final cancelled = orders.where((order) => order.status == OrderStatus.cancelled).length;
      final completionRate = orders.isEmpty ? 0 : (completed * 100 / orders.length).round();
    ...
    Widget _statCards(int total, int completed, int cancelled) {
      return Row(
        children: [
          Expanded(child: _statCard('Total Orders', '$total', Icons.receipt_long_outlined)),
          const SizedBox(width: 8),
          Expanded(child: _statCard('Completed Orders', '$completed', Icons.check_circle_outline)),
          const SizedBox(width: 8),
          Expanded(child: _statCard('Cancelled Orders', '$cancelled', Icons.cancel_outlined)),
        ],
      );
    }
    ...
    Widget _metricRow(int completionRate, int total) {
      ...
      return Row(
        children: [
          Expanded(child: _metricCard('$completionRate% completed', 'Completion rate')),
          if (average != null) ...[
    ...
    (String, String)? _averageLabel(int total) {
      switch (_selection.period) {
        ...
        case SuperAdminAnalyticsPeriod.month:
          final days = _selection.range.end.difference(_selection.range.start).inDays;
        ...
        case SuperAdminAnalyticsPeriod.custom:
          final days = _selection.range.end.difference(_selection.range.start).inDays;
          return ((total / math.max(days, 1)).toStringAsFixed(1), 'Average orders per day');
      }
    }
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
                ...
                child: SingleChildScrollView(
                  ...
                  child: Row(
                    ...
                    children: buckets.map((bucket) {
                      final height = bucket.value == 0 ? 2.0 : 150 * bucket.value / chartMax;
                      return Tooltip(
                          ...
                          child: SizedBox(
                            ...
                            child: Column(
                              ...
                              children: [
                                ...
                                Text(bucket.label, overflow: TextOverflow.ellipsis, style: const TextStyle(fontSize: 10)),
                              ],
                            ),
                          ),
                        ),
              ...
              Text('Scale: 0 - $chartMax orders', style: const TextStyle(color: AppTheme.textMuted, fontSize: 11)),
            ],
          ),
        ),
      );
                  ...
                  children: [
                    ...
                    Expanded(
                      child: Column(
                        crossAxisAlignment: CrossAxisAlignment.start,
                        children: [
                          const Text('Most Ordered Item', style: TextStyle(color: AppTheme.textSecondary)),
                          const SizedBox(height: 5),
                          Text(top.item.name, style: const TextStyle(fontSize: 18, fontWeight: FontWeight.w800)),
                          const SizedBox(height: 4),
                          Text('${top.quantity} ordered', style: const TextStyle(color: AppTheme.textSecondary)),
                        ],
                      ),
                    ),
                  ],
      ...
      return Card(
        child: Padding(
          ...
          child: Column(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              const Text('Top 5 Most Ordered Items', style: TextStyle(fontSize: 17, fontWeight: FontWeight.w800)),
              const SizedBox(height: 8),
              if (ranked.isEmpty)
                ...
                ...ranked.take(5).toList().asMap().entries.map((entry) {
                  ...
                  return ListTile(
                    ...
                    trailing: Text('${rankedItem.quantity}', style: const TextStyle(fontWeight: FontWeight.w800)),
                  );
                }),
            ],
          ),
    ...
    List<_RankedItem> _rankedItems(List<Order> orders) {
      ...
      for (final order in orders) {
        for (final item in order.items) {
          quantities[item.menuItemId] = (quantities[item.menuItemId] ?? 0) + item.quantity;
          names[item.menuItemId] = item.name;
        }
      }
      final ranked = quantities.entries.map((entry) {
        final menuItem = _menuItems.where((item) => item.id == entry.key).firstOrNull;
        return _RankedItem(
          menuItem ??
              MenuItem(
                id: entry.key,
